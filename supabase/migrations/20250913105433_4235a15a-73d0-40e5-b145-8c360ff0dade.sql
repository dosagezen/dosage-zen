...
BEGIN
  ...
  WHILE occurrence_utc <= day_end_utc LOOP
    ...
    calc_time := occurrence_local::TIME;
    
    -- Round to 15-minute blocks for fractional hours
    IF interval_hours != FLOOR(interval_hours) THEN
      hour_part := EXTRACT(hour FROM calc_time)::INT;
      minute_part := EXTRACT(minute FROM calc_time)::INT;
      
      -- Round minutes to nearest 15-minute block
      IF minute_part < 8 THEN
        minute_part := 0;
      ELSIF minute_part < 23 THEN
        minute_part := 15;
      ELSIF minute_part < 38 THEN
        minute_part := 30;
      ELSIF minute_part < 53 THEN
        minute_part := 45;
      ELSE
        minute_part := 0;
      ...
      calc_time := (hour_part || ':' || minute_part)::TIME;
    END IF;
    
    RETURN NEXT calc_time;
