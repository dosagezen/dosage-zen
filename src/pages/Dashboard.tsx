...
const Dashboard = () => {
  ...
  } = useAppointments('consulta', currentContext);
  const {
    appointments: exames
  } = useAppointments('exame', currentContext);
  const {
    total,
    concluidos,
    restantes
  ...
  const calcularProximoHorario = (horarios: any[]) => {
    const pendentes = horarios.filter(h => h.status === 'pendente' && h.hora !== '-').sort((a, b) => {
  ...
  const proximasMedicacoes = medicacoesAtivas.map(med => {
    ...
    return {
      ...
      status: status,
      proximoHorarioMinutos: proximoHorario ? proximoHorario.hora.split(':').map(Number).reduce((h, m) => h * 60 + m) : 9999 // Valor alto para colocar no final
    };
    ...
    return <div className="min-h-screen flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <div className="space-x-4">
            ...
            <Button onClick={() => setLoadingTimeout(false)} variant="ghost">
  ...
  return <div className="p-6 space-y-6 bg-gradient-soft min-h-screen">
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        return <Card key={index} className={`shadow-card hover:shadow-floating transition-shadow duration-300 ${isClickable ? 'cursor-pointer hover:scale-105 transition-transform' : ''}`} onClick={isClickable ? handleClick : undefined} aria-label={isClickable ? index === 0 ? `Restantes ${restantes}, Total ${total}, Concluídos ${concluidos}` : `Ir para ${stat.titulo}` : undefined}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="shadow-card">
          ...
          <CardContent className="space-y-4">
            {proximasMedicacoes.length > 0 ? proximasMedicacoes.map((med, index) => <div key={index} className="flex items-center justify-between p-3 bg-accent/10 rounded-lg hover-scale transition-all duration-200 hover:bg-accent/20">
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <Badge variant={med.status === 'pendente' ? 'secondary' : med.status === 'atrasado' ? 'destructive' : med.status === 'concluido' ? 'default' : 'outline'} className={`text-[10px] px-1.5 py-0.5 h-4 whitespace-nowrap ${med.status === 'pendente' ? "bg-orange-500 text-white hover:bg-orange-600" : med.status === 'atrasado' ? "bg-red-500 text-white hover:bg-red-600" : med.status === 'concluido' ? "bg-green-500 text-white hover:bg-green-600" : "bg-gray-500 text-white hover:bg-gray-600"}`}>
                      {med.status === 'pendente' ? 'Pendente' : med.status === 'atrasado' ? 'Atrasado' : med.status === 'concluido' ? 'Concluído' : 'Sem horário'}
                    </Badge>
                  </div>
        ...
        <Card className="shadow-card">
          ...
          <CardContent className="space-y-3">
            {compromissosProximos.length > 0 ? compromissosProximos.map((apt, index) => {
      ...
      <Card className="shadow-card">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Button variant="outline" className="h-16 flex-col gap-2 hover:bg-accent/20" onClick={() => {
            navigate('/app/agenda');
            window.scrollTo(0, 0);
